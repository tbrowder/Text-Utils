...
a in creating PostScript PDF or other output formats where blocks (e.g., paragraphs)
need to be wrapped to a specific maximum width based on the font
face and font size to be used.
Note it does not have all the options of
the B<wrap-paragraph> routine, but it will
wrap the input text differently for the first line
versus the remaining lines if desired.

All arguments except the C<:font> are in PostSript points (PS)
which are 72 per inch. The default C<:width> is 468 points,
...
This routine is ported from the Perl version in the I<The Perl Cookbook, 2e>.

This routine adds commas to a number to separate multiples of a thousand.
